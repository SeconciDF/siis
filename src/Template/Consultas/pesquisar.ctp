...
<table class="table table-bordered">
  ...
  <tbody  style="background: #fff;">
    <?php foreach ($pacientes as $paciente): ?>
      <tr>
        <td><?php echo $paciente['dependentes_id']?"(D) {$paciente['nome']}":$paciente['nome']; ?></td>
        ...
        <td style="width: 100px; text-align: center;">
          <?php echo $this->Html->link('<span class="icon-edit"></span> Marcar', ['controller' => 'consultas', 'action' => ($this->request->query('action') ? $this->request->query('action') : 'add'), $paciente['beneficiarios_id'], $paciente['dependentes_id']?$paciente['dependentes_id']:null], ['class' => 'btn', 'escape'=>false]); ?>
        </td>
      </tr>
    <?php endforeach; ?>
  </tbody>
