...
<div class="widgetcontent bordered shadowed nopadding">
  ...
  <?php if($paciente['cd_trab'] && ($this->request->query('medico') || $this->request->query('clinica') == '17')) { ?>
    <p>
      <label style="height: 50px; margin-right: 5px;">Selecione dia e turno </label>
      <span class="field" style=" padding:0;">
...
jQuery(document).ready(function() {
  ...
  jQuery('#clinica, #natureza, #funcao, .servico').change(function() {
    filtrar();
    lock();
  });
...
function filtrar() {
  url = '<?php echo $this->Url->build(['controller' => 'EmpresasAgendasMedicos', 'action' => 'siisv1-add', $paciente['cd_trab']], true); ?>?1=1';
