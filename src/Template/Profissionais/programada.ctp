<?php
    ...
    if($this->request->query('end') != $this->request->query('start')) {
        $profissional['data_fim'] = date('d/m/Y', strtotime($this->request->query('end')));
    }
?>


<h4 class="widgettitle nomargin shadowed">Agenda Programada - <?php echo $profissional['nome']; ?></h4>
...
<?php
    ...
    echo $this->Form->input('tipo', ['type' => 'hidden', 'value' => 'N']);
?>
...
<p>
    ...
    <span class="field">
        <small style="width: 85px;">Data in&iacute;cio</small><small>Hora in&iacute;cio</small>
        <?php
            if($this->request->query('end') != $this->request->query('start')) {
                echo '<small style="width: 85px;">Data fim</small>';
            }
        ?>

        <small>Hora fim</small><small>Per&iacute;odo</small><br/>
        ...
        <?php
            if($this->request->query('end') != $this->request->query('start')) {
                echo $this->Form->input('data_fim', ['label' => false, 'type' => 'text', 'required' => false, 'class' => 'input-small', 'placeholder' => 'dd/mm/aaaa']);
            }
        ?>
...
<style>.field small { min-width: 70px; display: inline-block; margin-right: 20px; }</style>
