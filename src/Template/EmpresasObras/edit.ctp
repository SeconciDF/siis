...
<p>
    ...
    <span class="field">
        ...
        <?php
            if($obra['situacao'] == 'A') echo "<button type='submit' onclick='javascript: $(\"#situacao\").val(\"I\");' style='color: #fff; border: 0; padding: 6px 15px 6px 15px; background: green;' title='Tornar inativo'>ATIVO</button>";
...
<p>
    ...
    <span class="field">
        ...
        <?php $obra->celular_contato ? $obra->celular_contato = formatFone($obra->celular_contato) : null; ?>
        <?php echo $this->Form->input('celular_contato', ['label' => false, 'type' => 'text', 'required' => false, 'class' => 'input-small fone']); ?>  &nbsp;&nbsp;&nbsp;
        <?php $obra->telefone_contato ? $obra->telefone_contato = formatFone($obra->telefone_contato) : null; ?>
