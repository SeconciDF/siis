...
<div class="widgetcontent bordered shadowed nopadding">
  <table class="table table-bordered">
      <thead>
          <tr>
              <th><?= $this->Paginator->sort('AmbientesSetores.descricao', 'Sequ&ecirc;ncia', ['escape' => false]); ?></th>
              <th><?= $this->Paginator->sort('AmbientesSetores.processo', 'Processo'); ?></th>
      ...
      <tbody  style="background: #fff;">
          ...
          <tr>
              <td style="width: 5%;"><?= h($processo['sequencia']); ?></td>
              <td style="width: 35%;"><?= h($processo['processo']); ?></td>
              <td><?= h($processo['descricao']); ?></td>
              <td style="width: 190px; text-align: center;">
                <?php echo $this->Html->link('<span class="icon-edit"></span> Editar', ['action' => 'edit', $processo['id']], ['class' => 'btn', 'escape'=>false]); ?>
                <?php echo $this->Form->postLink('<span class="iconfa-trash"></span> Deletar', ['action' => 'delete', $processo['id']], ['confirm' => 'Deseja excluir?', 'class' => 'btn btn-danger', 'escape'=>false]); ?>
              </td>
          </tr>
          <?php endforeach; ?>
      </tbody>
