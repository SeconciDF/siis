...
class AtendimentosController extends AppController {
  ...
  public function prontuario($id = null) {
    ...
    if(!$this->request->query('completo')) {
      $date = new \DateTime($consulta['data_hora_agendado']);
      $date->sub(new \DateInterval('P1Y'));
      $opt1[] = "Consultas.data_hora_agendado > '{$date->format('Y-m-d H:i:s')}'";
    }
    ...
    if(!$this->request->query('completo')) {
      $date = new \DateTime($consulta['data_hora_agendado']);
      $date->sub(new \DateInterval('P1Y'));
      $opt2[] = "Consulta.data_hora_agendado > '{$date->format('Y-m-d H:i:s')}'";
    }
  ...
  public function odontograma($id = null) {
    ...
    $consulta = $this->Consultas->find('all', [
      ...
      if ($this->request->session()->read('Auth.User.bloqueado')) {
        ...
        $ref = $this->Consultas->find('all', ['fields' => ['id','tratamento_alta_retorno'], 'conditions' => ['Consultas.id' => $consulta_id, 'st_consulta' => 'AC']])->first();
        if(!$ref && !$consulta['data_hora_atendimento']) {
          $opt=[
          ...
          $ref = $this->Consultas->find('all', [
            'conditions' => $opt,
            'fields' => [ 'id', 'tratamento_alta_retorno'],
            'order' => [
              'data_hora_agendado' => 'DESC'
            ]])->first();
        }

        if($ref['tratamento_alta_retorno'] || $consulta['data_hora_atendimento']) {
          $consulta_id = $consulta['id'];
        } else if($ref['id']) {
          $consulta_id = $ref['id'];
        }
